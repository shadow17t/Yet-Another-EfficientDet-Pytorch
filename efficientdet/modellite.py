...
class SeparableConvBlock(nn.Module):
    """
    created by Zylo117, modified by shadow17t
    """
...
class BiFPN(nn.Module):
    ...
    def _forward_fast_attention(self, inputs):
        ...
        p3_out = self.conv3_up(self.relu(weight[0] * p3_in + weight[1] * self.p3_upsample(p4_up)))
...
class SkipBiFPN(nn.Module):
    ...
    def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4, onnx_export=False, attention=True,
        ...
        self.p63_upsample = nn.Sequential(nn.Upsample(scale_factor=2, mode='nearest'), 
                                          nn.Upsample(scale_factor=2, mode='nearest'), 
                                          nn.Upsample(scale_factor=2, mode='nearest')) #upsample x3
        ...
        self.p53_downsample = nn.Sequential(MaxPool2dStaticSamePadding(3,2), 
                                            MaxPool2dStaticSamePadding(3,2)) #maxpool x2
        self.p64_downsample = nn.Sequential(MaxPool2dStaticSamePadding(3,2), 
                                            MaxPool2dStaticSamePadding(3,2)) #maxpool x2
        self.p75_downsample = nn.Sequential(MaxPool2dStaticSamePadding(3,2), 
                                            MaxPool2dStaticSamePadding(3,2)) #maxpool x2
        self.p74_downsample = nn.Sequential(MaxPool2dStaticSamePadding(3,2), 
                                            MaxPool2dStaticSamePadding(3,2), 
                                            MaxPool2dStaticSamePadding(3,2)) #maxpool x3
        ...
        self.p4_w1 = nn.Parameter(torch.ones(2, dtype=torch.float32), requires_grad=True)
        self.p4_w1_relu = nn.ReLU()
        self.p3_w1 = nn.Parameter(torch.ones(3, dtype=torch.float32), requires_grad=True) # +p6_1
        self.p3_w1_relu = nn.ReLU()

        self.p4_w2 = nn.Parameter(torch.ones(3, dtype=torch.float32), requires_grad=True)
        self.p4_w2_relu = nn.ReLU()
        self.p5_w2 = nn.Parameter(torch.ones(4, dtype=torch.float32), requires_grad=True) # +p3_out
        self.p5_w2_relu = nn.ReLU()
        self.p6_w2 = nn.Parameter(torch.ones(4, dtype=torch.float32), requires_grad=True) # +p4_out
        self.p6_w2_relu = nn.ReLU()
        self.p7_w2 = nn.Parameter(torch.ones(4, dtype=torch.float32), requires_grad=True) # +p4_1 +p5_out
        self.p7_w2_relu = nn.ReLU()
    ...
    def forward(self, inputs):
        """
        illustration of a minimal skip-bifpn unit
                                   -----------|
            P7_0 ------------------|------> P7_2 <------->
               |-------------|-----|--|       ↑      |
                             ↓     |  |       |      |
    ...
    def _forward_fast_attention(self, inputs):
        ...
        p3_w1 = self.p3_w1_relu(self.p3_w1)
        weight = p3_w1 / (torch.sum(p3_w1, dim=0) + self.epsilon)
        # Connections for P3_0 and P4_1&P6_1 to P3_2 respectively
        p3_out = self.conv3_up(self.relu(weight[0] * p3_in + weight[1] * self.p3_upsample(p4_up) 
                                         + weight[2] * self.p63_upsample(p6_in)))
        ...
        p5_w2 = self.p5_w2_relu(self.p5_w2)
        weight = p5_w2 / (torch.sum(p5_w2, dim=0) + self.epsilon)
        # Connections for P5_0, P5_1 and P4_2+P3_2 to P5_2 respectively
        p5_out = self.conv5_down(
            self.relu(weight[0] * p5_in + weight[1] * p5_up + weight[2] * self.p5_downsample(p4_out)
                      + weight[3] * self.p53_downsample(p3_out)))

        # Weights for P6_0, P6_1 and P5_2 to P6_2
        p6_w2 = self.p6_w2_relu(self.p6_w2)
        weight = p6_w2 / (torch.sum(p6_w2, dim=0) + self.epsilon)
        # Connections for P6_0, P6_1 and P5_2+P4_2 to P6_2 respectively
        p6_out = self.conv6_down(
            self.relu(weight[0] * p6_in + weight[1] * p6_up + weight[2] * self.p6_downsample(p5_out)
                      + weight[3] * self.p64_downsample(p4_out)))

        # Weights for P7_0 and P6_2 to P7_2
        p7_w2 = self.p7_w2_relu(self.p7_w2)
        weight = p7_w2 / (torch.sum(p7_w2, dim=0) + self.epsilon)
        # Connections for P7_0 and P6_2+P4_1+P5_2 to P7_2
        p7_out = self.conv7_down(self.relu(weight[0] * p7_in + weight[1] * self.p7_downsample(p6_out) 
                                           + weight[2] * self.p74_downsample(p4_up) + weight[3] * self.p75_downsample(p5_out)))
    ...
    def _forward(self, inputs):
        ...
        # Connections for P3_0 and P4_1+p6_1 to P3_2 respectively
        p3_out = self.conv3_up(self.relu(p3_in + self.p3_upsample(p4_up) + self.p63_upsample(p6_up)))
        ...
        # Connections for P5_0, P5_1 and P4_2+p3_2 to P5_2 respectively
        p5_out = self.conv5_down(
            self.relu(p5_in + p5_up + self.p5_downsample(p4_out) + self.p53_downsample(p3_out)))

        # Connections for P6_0, P6_1 and P5_2+p4_2 to P6_2 respectively
        p6_out = self.conv6_down(
            self.relu(p6_in + p6_up + self.p6_downsample(p5_out) + self.p64_downsample(p4_out)))
        ...
        if self.use_p8:
            ...
            # Connections for P7_0 and P6_2+p4_1+p5_2 to P7_2
            p7_out = self.conv7_down(self.relu(p7_in + self.p7_downsample(p6_out) + self.p74_downsample(p4_up) + self.p75_downsample(p5_out)))
